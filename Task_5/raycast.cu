...
//To be able to use these functions on the device, we need to add __host__ __device__
...
//The kernel function, should execute N timtes. but defined in the calling of the function.
__global__ void raycast_kernel(unsigned char* data, unsigned char* image, unsigned char* region){
//The part here is for defining the camera to sample the 3D image.	
...
__global__ void raycast_kernel_texture(unsigned char* image){
   // data and region is not a texture memory, how to fetch them would be to use 
   //
	// Camera/eye position, and direction of viewing. These can be changed to look
    // at the volume from different angles.
    float3 camera = {.x=1000,.y=1000,.z=1000};
...
//The part down here is the part where the camera go through the #D datasheet and sample the values.
	//Tihs code is the same as the raycasting kernel, difference is that it uses texture fetch and not just normal access.
	...
	// Move along the ray, we stop if the color becomes completely white,
            ...
            while(color < 255 && i < 5000){
                i++;
                pos = add(pos, scale(ray, step_size));          // Update position
				//Need to change here to fit the syntax for texture memory.
			   int r = value_at(pos, region);                  // Check if we're in the region
                color += value_at(pos, data)*(0.01 + r) ;       // Update the color based on data value, and if we're in the region
	}
	 // Write final color to image
...
//This is the function that would be called. It should allocate the needed space here before calling the kernel function.
//By calling cudaMalloc the system will allocate the space in the device's memory. 
unsigned char* raycast_gpu(unsigned char* data, unsigned char* region){	
	//declare variable for local usage.
	int total_pixels = IMAGE_DIM * IMAGE_DIM;
    unsigned char* image = (unsigned char*)malloc(sizeof(unsigned char)*total_pixels);
	
	//Allocate the memory on the device(GPU)
	unsigned char* d_data;
	cudaMalloc(&d_data, sizeof(unsigned char) * DATA_DIM*DATA_DIM*DATA_DIM);

	unsigned char* d_region;
	cudaMalloc(&d_region, sizeof(unsigned char)* DATA_DIM*DATA_DIM*DATA_DIM);
...
//This function uses a special memory type called texture. In theory, it should be much faster since the advantages of texture memory is faster access 
//during accessing a special pattern.
unsigned char* raycast_gpu_texture(unsigned char* data, unsigned char* region){

int total_pixels = IMAGE_DIM * IMAGE_DIM;
    unsigned char* image = (unsigned char*)malloc(sizeof(unsigned char)*total_pixels);
//since the image need to be written to, we can't use texture memory since it is only for read.
unsigned char* d_image; 
	cudaMalloc(&d_image, sizeof(unsigned char)*total_pixels);
		
/**Using texture memory, we need to follow a certain steps, 
- declare texture memory in cuda.
- bind the texture mem to our refrence texture mem.
- read the texture mem from our refrence 
- Unbind the texture mem from our texture ref.
**/

//declare a texture for reference.
